...
class SalonDatabase {
  ...
  void insrtAppoinment(Appointment appointment) {
    sqlite.execute(
      'INSERT OR REPLACE INTO appointments(id,clientId,serviceId,start) VALUES(?,?,?,?)',
  ...
  void updateClient(Client client) {
    sqlite.execute('UPDATE clients SET name=?,phone=? WHERE id=?', [
      client.id,
      client.name,
      client.phone,
    ]);
  ...
  void updateService(ServiceSalon service) {
    sqlite.execute('UPDATE services SET title=?,price=?,durationTime=? WHERE id=?', [
      service.id,
      service.title,
      service.price,
      service.durationTime,
  ...
  void updateAppointment(Appointment appointment) {
    sqlite.execute('UPDATE appointments SET clientId=?,serviceId=?,start=? WHERE id=?', [
      appointment.id,
...
void main() {
  ...
  db._createTable();
  final client = Client(id: "1", name: "антон", phone: "42");
